...
export class BaseNavbarComponent implements OnInit, OnDestroy, AfterViewInit {
	...
	this.attachLsidernavEventListeners();
    }
    ...
    toggleSidebar(): void {
	 ...
	 private attachLsidernavEventListeners(): void {
        
        $(document).ready(function () {
            var searchField = $('#search-modules');
            var lsidernav = $('.lsidernav');
            var hideonlve = $('.hideonlve');
            var scrollside = $('.scrollside');

        
            lsidernav.on('mouseenter', function() {
                // console.log('Mouse entered lsidernav');
                $(this).addClass('dropdown-open');
                searchField.focus();
                lsidernav.css('transition', 'all 1s ease');
                lsidernav.css('width', '14rem');
                scrollside.css('text-wrap','balance');
                hideonlve.css('display', 'block');
                searchField.focus();
                     
            });
            
            $(document).ready(function () {
                $(document).on('mouseenter', '.parentm.nav-item', function () {
                    var menuItem = $(this);
                    var submenuWrapper = $('.tmenu', menuItem);
                    var menuItemPos = menuItem.position();
                    submenuWrapper.css({
                        top: menuItemPos.top - Math.round(menuItem.outerHeight() * 0.5),
                        // left: menuItemPos.left + Math.round(menuItem.outerWidth() * -0.5)
                        });
                });
            });
        
            // lsidernav.on('mouseleave', function() {
            //     // console.log('Mouse left lsidernav');
            //     $(this).removeClass('dropdown-open');
            //     lsidernav.css('width', '4rem');
            //     lsidernav.css('transition', 'all 1s ease');
            //     hideonlve.css('display', 'none');
            //     scrollside.css('text-wrap','nowrap');
            //     clearsinput();
            // });
        
            // $(document).on('mouseover', function(event) {
            //     if (!lsidernav.is(event.target) && lsidernav.has(event.target).length === 0) {
            //         // console.log('Mouse not over lsidernav');
            //         searchField.blur();
            //         $(this).removeClass('dropdown-open');
            //         scrollside.css('text-wrap','nowrap');
            //     }
            // });
            lsidernav.on('mouseenter', function() {
                console.log('Mouse entered lsidernav mouseenter');
                if (lsidernav.width() === 4) {
                console.log('imsiwde 4 field');
                $(this).addClass('dropdown-open');
                searchField.focus();
                lsidernav.css('transition', 'all 1s ease');
                lsidernav.css('width', '14rem');
            ...
            function handleMouseLeaveAndClick() {
                console.log('imsiwde 12 field');
                $(this).removeClass('dropdown-open');
                lsidernav.css({
                    'width': '4rem',
            ...
            lsidernav.on('mouseleave click', handleMouseLeaveAndClick);
...
declare function searchModules(): void;
